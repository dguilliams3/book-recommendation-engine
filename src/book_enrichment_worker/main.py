...
LLM_MODEL = os.getenv(
    "BOOK_DESC_LLM_MODEL", "gpt-4o-mini"
...
async def _llm_description_with_retry(
    title: str, author: str | None, genres: list[str], year: str | int | None
) -> str | None:
...
async def llm_description(
    ...
    try:
        ...
        logger.warning(
            "OpenAI API connection error", extra={"title": title, "error": str(e)}
        )
        return None
    except APITimeoutError as e:
        logger.warning("OpenAI API timeout", extra={"title": title, "error": str(e)})
        return None
    except OpenAIError as e:
        logger.error(
            "OpenAI API error",
            extra={"title": title, "error": str(e), "error_type": type(e).__name__},
        )
        return None
    except Exception as e:
        logger.error(
            "Unexpected error in description generation",
            exc_info=True,
...
async def google_books(isbn, session):
    ...
    try:
        async with session.get(url) as r:
            if r.status != 200:
                logger.warning(
                    "Google Books API returned non-200 status",
                    extra={"isbn": isbn, "status": r.status, "url": url},
                )
                return {}
        ...
        logger.error(
            "Failed to fetch from Google Books",
            exc_info=True,
            extra={"isbn": isbn, "url": url},
        )
        return {}
...
async def open_library(isbn, session):
    ...
    try:
        async with session.get(url) as r:
            if r.status != 200:
                logger.warning(
                    "Open Library API returned non-200 status",
                    extra={"isbn": isbn, "status": r.status, "url": url},
                )
                return {}
...
MAX_CONCURRENT_REQUESTS = int(
    os.getenv("BOOK_DESC_MAX_CONCURRENT", "8")
)  # Rate limiting


async def process_book_descriptions_batch(
    ...
    async def process_single_book(book: Dict[str, Any]) -> tuple[str, str | None]:
        ...
        async with semaphore:
            ...
            try:
                ...
                logger.warning(
                    "Failed to process book description",
    ...
    logger.info(
        "Batch description processing completed",
        extra={
...
async def enrich_all_missing_descriptions():
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    f"Bulk enrichment DB connection attempt {attempt + 1} failed, retrying in {retry_delay}s",
                    extra={
                        "attempt": attempt + 1,
                ...
                logger.error(
                    "Failed to connect to database for bulk enrichment after all retries",
                    exc_info=True,
                )
                raise
    ...
    try:
                ...
                if description is not None:
                    ...
                    try:
                        upd_evt = BookUpdatedEvent(
                            book_id=book_id, payload={"description_added": True}
                        )
                        await publish_event(BOOK_EVENTS_TOPIC, upd_evt.model_dump())
                    except Exception:
                        logger.warning(
                            "Failed to publish book updated event",
        ...
        logger.info(
            "Bulk description enrichment completed",
            extra={
                "total_processed": total_processed,
                "total_updated": total_updated,
                "success_rate": (
                    f"{(total_updated/total_processed)*100:.1f}%"
...
async def main():
    ...
    logger.info(
        "Rate limits",
        extra={"google_books_rpm": GB_PER_MIN, "open_library_rpm": OL_PER_MIN},
    )
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    "Failed to connect to database after all retries", exc_info=True
                )
                raise
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=20)) as ses:
        ...
        for row in books:
            ...
            if isbn:
                logger.debug(
                    "Attempting Google Books enrichment",
                ...
                if meta:
                    ...
                    logger.debug(
                        "Google Books enrichment successful", extra={"book_id": book_id}
                    )

            # Fallback to Open Library
            if not meta and isbn:
                logger.debug(
                    "Attempting Open Library enrichment",
                    extra={"book_id": book_id, "isbn": isbn},
                )
                async with sem_ol:
                ...
                if meta:
                    ...
                    logger.debug(
                        "Open Library enrichment successful", extra={"book_id": book_id}
                    )
            ...
            if meta:
                vi = (
                    meta.get("items", [{}])[0].get("volumeInfo", {})
                ...
                if page is not None and (not isinstance(page, int) or page <= 0):
                    logger.debug(
                        "Invalid page count received, treating as None",
                        extra={"book_id": book_id, "invalid_page_count": page},
                    )
                    page = None
            ...
            try:
                await conn.execute(
                    ...
                    description,
                    rb.get("difficulty_band"),
                    book_id,
                )

                enriched_count += 1
                logger.debug(
                    "Database updated successfully", extra={"book_id": book_id}
                )
                ...
                try:
                    upd_evt = BookUpdatedEvent(
                        ...
                        payload={
                            "page_count": page,
                            "publication_year": year,
                            "description_added": bool(description),
                        },
                    )
                    await publish_event(BOOK_EVENTS_TOPIC, upd_evt.model_dump())
                except Exception:
    ...
    await conn.close()
    logger.info(
        "Book enrichment process completed",
        extra={
