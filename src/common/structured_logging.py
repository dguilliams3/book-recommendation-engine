...
feature_flags_var: ContextVar[Optional[Dict[str, Any]]] = ContextVar(
...
def get_logger(
    name: str | None = None, kafka_producer: Optional["AIOKafkaProducer"] = None
):
    ...
    class KafkaJsonFormatter(logging.Formatter):
        ...
        _builtin_keys = set(
            logging.LogRecord(None, 0, "", 0, "", (), None, None).__dict__.keys()
        )
            ...
            payload.update(
                {
                    ...
                    "reader_mode_enabled": os.getenv("ENABLE_READER_MODE", "false")
                    == "true",
                }
            )
    ...
    if LOG_TO_KAFKA and AIOKafkaProducer is not None:
        ...
        if kafka_producer and not any(
            isinstance(h, KafkaLogHandler) for h in logger.handlers
        ):
            try:
...
def log_api_request(
    logger: logging.Logger, method: str, path: str, user_id: str = None, **context
):
...
def log_business_event(
    logger: logging.Logger, event_type: str, entity_id: str = None, **context
):
