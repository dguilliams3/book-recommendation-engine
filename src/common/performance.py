...
class PerformanceMonitor:
    ...
    async def monitor_query(self, query_type: str):
        ...
        try:
            ...
            # Log slow queries
            if self.config.profile_slow_queries and duration * 1000 > self.config.slow_query_threshold_ms:
...
async def warm_up_caches() -> None:
    ...
    warmup_data = {
        'popular_genres': ['Fiction', 'Non-Fiction', 'Mystery', 'Romance', 'Science Fiction'],
...
async def performance_context(operation_name: str):
    ...
    async with monitor.monitor_query(operation_name):
        ...
        try:
            ...
            logger.info(f"Performance summary for {operation_name}", extra={
