...
def compute_student_reading_level(
    ...
    Examples:
        ...
        6.0  # 4 + 2
    """
    logger.debug("Computing student reading level", extra={
    ...
    for row in checkout_rows[-recent_limit:]:  # Take last N checkouts
        if row.get('reading_level') is not None:
            try:
                ...
                logger.debug("Skipping invalid reading level", extra={"value": row.get('reading_level')})
    ...
    if reading_levels:
        ...
        confidence = min(len(reading_levels) / 5.0, 1.0)  # More books = higher confidence
    ...
    try:
        ...
        logger.warning("Invalid EOG/grade data, using grade fallback", extra={
            "student_grade": student_grade, 
            "eog_score": eog_score,
            "error": str(e)
        })
...
async def get_student_reading_level_from_db(
    ...
    """
    logger.info("Fetching student reading level from database", extra={
    ...
    try:
        async with db_pool.acquire() as con:
            ...
            if not student_data:
                logger.warning("Student not found in database", extra={"student_id": student_id})
                return {
            ...
            logger.debug("Fetched student data and checkout history", extra={
                "student_id": student_id,
        ...
        logger.error("Database error computing student reading level", 
                    exc_info=True, extra={"student_id": student_id})
