...
async def get_student_reading_level(student_id: str):
    ...
    try:
        ...
        logger.info("Student reading level tool completed", extra={
            "student_id": student_id,
        ...
        logger.error("Reading level utils import failed", exc_info=True, extra={"student_id": student_id})
        return {
        ...
        logger.error("Unexpected error in reading level tool", exc_info=True, extra={"student_id": student_id})
        return {
            "avg_reading_level": 4.0,
            "confidence": 0.0,
...
async def query_students(student_id: str = None, grade_level: int = None, homeroom_teacher: str = None, limit: int = 10):
    """
    ...
    """
    logger.info("Students table query requested", extra={
        "student_id": student_id,
    ...
    try:
        ...
        async with pool.acquire() as con:
            ...
            logger.info("Students table query completed", extra={
...
async def query_catalog(book_id: str = None, genre: str = None, difficulty_band: str = None, min_rating: float = None, limit: int = 10):
    """
    ...
    """
    logger.info("Catalog table query requested", extra={
        "book_id": book_id,
        "genre": genre,
    ...
    try:
        ...
        async with pool.acquire() as con:
            ...
            where_sql = f"WHERE {' AND '.join(where_clauses)}" if where_clauses else ""
            query = f"SELECT * FROM catalog {where_sql} ORDER BY book_id LIMIT ${param_idx}"
            params.append(limit)
            
            rows = await con.fetch(query, *params)
            results = [dict(row) for row in rows]
            
            logger.info("Catalog table query completed", extra={
                "filters_applied": len(where_clauses),
                "results_count": len(results)
            })
...
async def query_checkout_history(student_id: str = None, book_id: str = None, include_returns_only: bool = False, limit: int = 20):
    """
    ...
    """
    logger.info("Checkout history query requested", extra={
        "student_id": student_id,
        "book_id": book_id,
    ...
    try:
        ...
        async with pool.acquire() as con:
            ...
            logger.info("Checkout history query completed", extra={
                "filters_applied": len(where_clauses),
                "results_count": len(results)
            })
...
async def query_student_similarity(student_a: str = None, student_b: str = None, min_similarity: float = None, limit: int = 15):
    """
    ...
    """
    logger.info("Student similarity query requested", extra={
        "student_a": student_a,
        "student_b": student_b,
    ...
    try:
        ...
        async with pool.acquire() as con:
            ...
            logger.info("Student similarity query completed", extra={
                "filters_applied": len(where_clauses),
                "results_count": len(results)
            })
