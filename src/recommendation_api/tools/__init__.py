...
def _merge_metadata(
    google_data: Optional[Dict], open_library_data: Optional[Dict]
) -> Optional[Dict]:
    """
...
async def enrich_book_metadata(
    ...
    if parallel_fetch:
        ...
        try:
            ...
            results = await asyncio.gather(
                google_task, open_library_task, return_exceptions=True
            )
        ...
        try:
            open_library_data = await fetch_open_library_meta(
                clean_isbn, use_cache=use_cache
            )
        except Exception as e:
            logger.warning(
                "Open Library fetch failed", extra={"isbn": clean_isbn, "error": str(e)}
            )
    ...
    if merged_metadata and author:
        ...
        author_match = any(
            author.lower() in fa or fa in author.lower() for fa in fetched_authors
        )
        if not author_match:
            logger.warning(
...
async def enrich_multiple_books(
    ...
    async def enrich_single_book(book: Dict[str, Any]) -> Dict[str, Any]:
        async with semaphore:
            ...
            if not isbn:
                logger.warning("Skipping book without ISBN", extra={"book": book})
                return book
    ...
    enriched_books = await asyncio.gather(
        *[enrich_single_book(book) for book in book_list], return_exceptions=True
    )
