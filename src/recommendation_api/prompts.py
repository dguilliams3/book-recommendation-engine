...
_STUDENT_SYSTEM_TMPL = _BASE_SYSTEM_TMPL + """
...
_READER_SYSTEM_TMPL = _BASE_SYSTEM_TMPL + """
...
def build_student_prompt(
    ...
    context_parts = [
        ...
        f"Average reading level: {avg_level:.1f}" if avg_level else "Average reading level: Unknown",
        f"Recent books: {', '.join(recent_titles[:5])}" if recent_titles else "No recent books",
        f"Top genres: {', '.join(top_genres)}" if top_genres else "No genre preferences",
    ]
...
def build_reader_prompt(
    ...
    feedback_text = ""
    if feedback_scores:
        feedback_items = [f"{book_id}: {score}" for book_id, score in list(feedback_scores.items())[:5]]
