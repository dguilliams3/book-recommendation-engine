...
class ReaderModeConfig:
    ...
    def from_env(cls) -> "ReaderModeConfig":
        ...
        return cls(
            ...
            READING_LEVEL_WEIGHT_HIGH=float(
                os.getenv("READER_READING_LEVEL_WEIGHT_HIGH", "1.5")
            ),
            READING_LEVEL_WEIGHT_MEDIUM=float(
                os.getenv("READER_READING_LEVEL_WEIGHT_MEDIUM", "1.0")
            ),
            READING_LEVEL_WEIGHT_LOW=float(
                os.getenv("READER_READING_LEVEL_WEIGHT_LOW", "0.5")
            ),
            ...
            FALLBACK_RATING_THRESHOLD=float(
                os.getenv("READER_FALLBACK_RATING_THRESHOLD", "3.5")
            ),
            ENABLE_BOOK_ENRICHMENT=os.getenv(
                "READER_ENABLE_BOOK_ENRICHMENT", "true"
            ).lower()
            == "true",
            ENABLE_FEEDBACK_SCORING=os.getenv(
                "READER_ENABLE_FEEDBACK_SCORING", "true"
            ).lower()
            == "true",
            ENABLE_QUERY_MATCHING=os.getenv(
                "READER_ENABLE_QUERY_MATCHING", "true"
            ).lower()
            == "true",
        )
