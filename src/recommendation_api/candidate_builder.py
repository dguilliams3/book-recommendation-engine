...
def _load_faiss_store() -> Optional[FAISS]:
    ...
    try:
        ...
        if not (vector_dir / "index.faiss").exists():
            logger.info("FAISS index not found, creating empty index", extra={"path": str(vector_dir)})
            # Create empty index so system can work
            vector_dir.mkdir(parents=True, exist_ok=True)
            store = FAISS.from_texts(["dummy"], embeddings, metadatas=[{"book_id": "dummy"}])
...
async def _semantic_book_candidates(student_id: str, k: int = 20) -> List[str]:
    ...
    try:
        ...
        if not rated_books:
            logger.debug("No rated books found for semantic search", extra={"student_id": student_id})
            return []
        ...
        for book_record in rated_books:
            ...
            try:
                ...
                logger.debug("Could not find embedding for book", extra={"book_id": book_id})
                continue
        
        if not weighted_embeddings or total_weight == 0:
            logger.debug("No valid embeddings found for aggregation", extra={"student_id": student_id})
            return []
        ...
        docs_with_scores = store.similarity_search_by_vector(query_vector, k=k*2)  # Oversample
        
        # Extract book_ids, filtering out already-read books
        already_read = await conn.fetch("SELECT book_id FROM checkout WHERE student_id=$1", student_id)
        ...
        logger.warning("Semantic search failed", exc_info=True, extra={"student_id": student_id})
        return []
    finally:
        await conn.close()


async def _query_based_semantic_candidates(query: str, student_id: str = None, k: int = 10, conn=None) -> List[str]:
    ...
    """
    logger.debug("Starting query-based semantic search", extra={"query": query, "student_id": student_id, "target_candidates": k})
    ...
    try:
        ...
        if student_id and conn:
            try:
                ...
                if context_rows:
                    ...
                    levels = [r["reading_level"] for r in context_rows if r["reading_level"]]
                    ...
                    if context_parts:
                        ...
                        logger.debug("Enhanced query with student context", extra={
                ...
                logger.debug("Failed to enhance query with student context", exc_info=True)
        ...
        logger.warning("Query-based semantic search failed", exc_info=True, extra={"query": query})
        return []


async def build_candidates(student_id: str, k: int = 30, query: str = None) -> List[CandidateBook]:
    ...
    read_rows = await conn.fetch("SELECT book_id FROM checkout WHERE student_id=$1", student_id)
    already_read = {r["book_id"] for r in read_rows}

    # Quick sanity – unknown student?  Raise so the API can return 404
    if not read_rows:
        prof = await conn.fetchrow("SELECT 1 FROM students WHERE student_id=$1", student_id)
        if prof is None:
            await conn.close()
    ...
    if query and query.strip():
        query_book_ids = await _query_based_semantic_candidates(query.strip(), student_id=student_id, k=min(k//3, 10), conn=conn)
    ...
    try:
        ...
        logger.debug("Nearest neighbours fetched", extra={"student_id": student_id, "neighbours": nbr_ids})
        if nbr_ids:
        ...
        logger.debug("Neighbour recent checkout aggregation", extra={"counts": len(nbr_recent_counts)})
    except Exception:
    ...
    for book_id in query_book_ids:
        ...
        try:
            conn = await _pg_connect()
            meta_row = await conn.fetchrow("SELECT title, reading_level AS level, author FROM catalog WHERE book_id = $1", book_id)
            await conn.close()
    ...
    if not out:
        try:
            ...
            await conn.close()
            for p in popular:
                if p["book_id"] not in already_read and not await was_recommended(student_id, p["book_id"]):
                    out.append(
                        {
                            "book_id": p["book_id"],
                            "title": None,
