...
def score_candidates(cands: Iterable[CandidateBook]) -> list[Tuple[float, CandidateBook]]:
    ...
    logger.debug("Scoring candidates", extra={"candidate_count": len(list(cands)), "weights": w})
    ...
    for c in cands:
        ...
        if c.get("level") is not None and c.get("student_level") is not None:
            diff = abs(float(c["level"]) - float(c.get("student_level", c["level"])))  # Convert to float
            reading_match = max(0, 1 - diff / 5)
            s += w.get("reading_match_weight", w.get("reading_match", 0.4)) * reading_match
