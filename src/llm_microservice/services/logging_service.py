...
class LoggingService:
    ...
    def log_request(
        ...
        try:
            ...
            sanitized_response = (
                self._sanitize_log_data(response_data) if response_data else None
            )
        ...
        try:
            ...
            if self.enabled:
                try:
                    ...
                    logger.warning(
                        f"Kafka metrics logging failed, falling back to file: {e}"
                    )
                    self.stats["kafka_failures"] += 1
    ...
    def log_error(
        ...
        try:
            ...
            if self.enabled:
                try:
                    self._send_to_kafka(
                        topic=self.config.error_topic, message=log_entry, key=request_id
                    )
                    ...
                    logger.warning(
                        f"Kafka error logging failed, falling back to file: {e}"
                    )
                    self.stats["kafka_failures"] += 1
