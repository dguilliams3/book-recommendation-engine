...
class CacheService:
    ...
    def delete(self, request_id: str) -> bool:
        ...
        if self.enabled:
            try:
                if self.redis_client:
                    ...
                    success = deleted > 0
                    if success:
                        logger.debug(f"Deleted cached response for request {request_id} from Redis")
            except Exception as e:
        ...
        if cache_key in self.fallback_cache:
            del self.fallback_cache[cache_key]
            logger.debug(f"Deleted cached response for request {request_id} from memory")
            success = True
    ...
    def clear_expired(self) -> int:
        ...
        if expired_keys:
            logger.debug(f"Cleared {len(expired_keys)} expired entries from memory cache")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.enabled and self.redis_client:
            try:
                ...
                stats['redis_connected_clients'] = redis_info.get('connected_clients', 'unknown')
            except Exception as e:
