...
class TestInvokeEndpoint:
    ...
    def test_invoke_success(
        self, mock_llm_service, client, sample_request, sample_response
    ):
    ...
    @patch("src.llm_microservice.main.llm_service")
    def test_invoke_cached_response(
        self, mock_llm_service, client, sample_request, sample_response
    ):
...
class TestStatsEndpoint:
    ...
    @patch("src.llm_microservice.main.llm_service")
    def test_stats_includes_service_stats(
        self, mock_llm, mock_logging, mock_cache, client
    ):
...
class TestErrorHandling:
    ...
    def test_json_parse_error(self, client):
        ...
        response = client.post(
            "/invoke", data="invalid json", headers={"Content-Type": "application/json"}
        )
        assert response.status_code == 422
