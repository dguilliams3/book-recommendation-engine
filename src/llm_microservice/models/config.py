...
class LLMConfig(BaseModel):
    ...
    retry_delay_base: float = Field(1.0, ge=0.1, le=10.0, description="Base delay for exponential backoff")
    retry_delay_max: float = Field(60.0, ge=1.0, le=300.0, description="Maximum delay between retries")
    
    # Timeout configuration
    request_timeout: int = Field(30, ge=5, le=300, description="Request timeout in seconds")
    connection_timeout: int = Field(10, ge=1, le=60, description="Connection timeout in seconds")
...
class RedisConfig(BaseModel):
    ...
    connection_timeout: int = Field(default=5, ge=1, le=30, description="Connection timeout in seconds")
...
class KafkaConfig(BaseModel):
    ...
    request_topic: str = Field(default="llm-requests", description="Topic for request logs")
    ...
    producer_timeout: int = Field(default=10, ge=1, le=60, description="Producer timeout in seconds")
    max_retries: int = Field(default=3, ge=0, le=10, description="Maximum producer retries")
...
class ServiceConfig(BaseModel):
    ...
    log_file: Optional[str] = Field(default=None, description="Log file path (fallback for Kafka)")
    ...
    default_max_tokens: int = Field(default=1000, ge=1, le=4000, description="Default max tokens")
