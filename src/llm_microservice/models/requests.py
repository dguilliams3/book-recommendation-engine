...
class PreProcessedPrompt(BaseModel):
    ...
    injection_protected: bool = Field(True, description="Whether injection protection was applied")
...
class LLMRequest(BaseModel):
    ...
    user_prompt: str = Field(..., min_length=1, max_length=10000, description="User's prompt/message")
    openai_api_key: str = Field(..., pattern=r"^sk-[a-zA-Z0-9]{32,}$", description="OpenAI API key")
    
    # Optional fields with defaults
    system_prompt: Optional[str] = Field(None, max_length=5000, description="System instructions (optional)")
    model: ModelName = Field(default=ModelName.GPT_4O_MINI, description="OpenAI model to use")
    ...
    pre_processed_prompt: Optional[PreProcessedPrompt] = Field(None, description="Pre-processed prompt")
    messages: Optional[List[LangChainMessage]] = Field(None, description="Structured LangChain messages")
    
    # Prompt template configuration (legacy)
    prompt_template: Optional[Dict[str, Any]] = Field(None, description="Custom prompt template config")
