...
async def process_feedback_event(event_data: Dict[str, Any]):
    ...
    try:
        ...
        async with async_session() as session:
            ...
            if user is None:
                logger.warning(
                    ...
                    extra={"user_hash_id": event.user_hash_id},
                )
                return

            # Store feedback in database
            feedback = Feedback(
                user_id=user.id, book_id=event.book_id, score=event.score
            )
            session.add(feedback)
            await session.commit()

            logger.info(
                "Stored feedback in database",
                extra={
                    "user_id": str(user.id),
                    "book_id": event.book_id,
                    "score": event.score,
        ...
        logger.info(
            "Updated Redis feedback cache",
            extra={"cache_key": cache_key, "score": event.score},
        )
...
async def main():
    ...
    consumer = KafkaEventConsumer(
        topic=FEEDBACK_EVENTS_TOPIC, group_id="feedback_worker"
    )
    ...
    try:
        ...
        await asyncio.gather(
            consumer_task, shutdown_event.wait(), return_exceptions=True
        )
